...
class PRBNPMIntegrator(RBIntegrator):
    ...
    def sample(self,
        ...
        prev_scatter_pdf = mi.Float(1.0)
        prev_scatter_delta = mi.Bool(True)

        class Dist:
            def __init__(self, bsdf) -> None:
                self.bsdf = bsdf

            def sample(self, si, sample1, sample2, active):
                return self.bsdf.sample(bsdf_ctx, si, sample1, sample2, active)
...
mi.register_integrator("prb_npm", lambda props: PRBNPMIntegrator(props))
